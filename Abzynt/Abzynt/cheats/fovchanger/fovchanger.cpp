...
{
	if (g_pconfig->settings.fovchanger)
	{
		...
		{
			if (local_fov != g_pconfig->settings.fovchanger_amount) // We're checking for current fov, so we will decrease amounts of wpms when fov is 90.
			{
				g_pmemory->write<int>(local_player->get_base() + offsets.fov, g_pconfig->settings.fovchanger_amount); // We're writting our desired fov.
			}
		}
